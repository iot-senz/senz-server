...
class mySensorUDPServer(DatagramProtocol):
    # This method will create a new user at the server based on the
    # following SENZE
    # SHARE #pubkey PEM_PUBKEY @mysensors #time timeOfRequest ^userName
    # signatureOfTheSenze
    def createUser(self, query, address):
        ...
        if cry.verifySENZE(query, pubkey):
            status = usr.addUser(query.getSender(), phone,
    ...
    # This methid will remove the user at the server based on
    # the following SENZE
    # UNSHARE #pubkey @mysensors #time timeOfRequest ^userName
    # signatureOfTheSenze
    ...
    def shareSensors(self, query):
        ...
        """
         ...
         Then mysensors adds "user2" to the sensor dictionary at user1's
         document and sensor name to the "user1" dictionary at user2's
         document.
