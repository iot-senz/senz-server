...
class myUser:
    ...
    def addUser(self, name, phone, senze, pubkey, signature):
        ...
        if (doc):
            ...
            # It saves public key
            user = {"name": name, "phone": phone, "senze": senze,
    ...
    def share(self, recipient, sensors):
        ...
        for sensor in sensors:
            ...
            if self.name in doc.keys():
                if sensor not in doc[self.name]:
            ...
            if self.isShare(recipient, [sensor]):
                ...
                if self.name in doc.keys():
                    if sensor in doc[self.name]:
                        ...
                        # If so remove the sensor name from the recipient
                        # dictionary
    ...
    def isShare(self, recipient, sensors):
        if not (self.usrDoc):
            return False
        for sensor in sensors:
            if sensor not in self.usrDoc.keys():
                return False
            if recipient not in self.usrDoc[sensor]:
                return False
        return True
    ...
    def isAllow(self, sender, sensors):
        if not (self.usrDoc):
            return False
        if sender not in self.usrDoc.keys():
            return False
        for sensor in sensors:
            if sensor not in self.usrDoc[sender]:
                return False
        return True
